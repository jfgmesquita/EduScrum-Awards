...
public interface AwardAssignmentRepository extends JpaRepository<AwardAssignment, Long> {
    ...
    @Query("SELECT s.id as studentId, s.name as studentName, COALESCE(SUM(a.points), 0) as totalScore " +
            "FROM TeamMember tm " +
            "JOIN tm.student s " +
            "LEFT JOIN AwardAssignment aa ON aa.student = s AND aa.team.id = :teamId " + // Prémios ganhos
                                                                                         // NESTA equipa
            "LEFT JOIN aa.award a " +
    ...
    /**
     * Finds all AwardAssignments for a given student, ordered by assignment date
     * descending.
     * 
     ...
     * 
     * @param degreeId the ID of the degree
     * @return list of RankingItemDTO with student names and their scores
     */
     ...
     * 
     * @param courseId the ID of the course
     * @return list of RankingItemDTO with team names and their average scores
     */
    @Query("SELECT new com.group7.eduscrum_awards.dto.RankingItemDTO(" +
            "   t.name, " +
    ...
    /**
     * Counts the number of AwardAssignments for a specific student filtered by
     * AwardType.
     * 
     * @param studentId the ID of the student
     * @param type      the type of award (MANUAL or AUTOMATIC)
     * @return the count of AwardAssignments of the specified type for the student
     */
    @Query("SELECT COUNT(aa) FROM AwardAssignment aa WHERE aa.student.id = :studentId AND aa.award.type = :type")
    int countByStudentAndType(@Param("studentId") Long studentId, @Param("type") AwardType type);

    /**
     * Calculates the total score for a specific student by summing the points of
     * all their awarded awards.
     * 
     * @param studentId the ID of the student
     * @return the total score of the student
     */
    @Query("SELECT COALESCE(SUM(a.points), 0) FROM AwardAssignment aa JOIN aa.award a WHERE aa.student.id = :studentId")
    long calculateTotalScore(@Param("studentId") Long studentId);
}